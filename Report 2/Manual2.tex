...
\begin{question}
    ...
    \begin{subquestion}{Discuss the observations.}
        \answer{
            \paragraph*{}
            At first, because the wire has very little resistance, a lot of current must pass through it, so the DC power supply switches to C.C mode and passes $0A$, which is equivalent to open-circuit. After that, when the current value is set to 0.5, it causes the DC power supply to switch to C.C mode again, but this time our circuit (which is a short circuit) has a current of $0.5A$. (which can lead to damage to the DC power supply)
        }
    \end{subquestion}
    ...
    \begin{subquestion}{Can you propose an experiment to measure the resistance of the alligator clip wire?}
        \answer{
            ...
            and $(I)$ is the current. By measuring the voltage of the wire 
            when DC power supply is on C.C mode with known current, we can substitute these values into
